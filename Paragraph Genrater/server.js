...
form.addEventListener("submit",function(e){
    ...
    if(isNaN(value) || value <= 0 || value > 9){
        result.innerHTML = `<p class="result w-[70%] font-light mt-4 text-xl">${text[random]}</p>`;
    }
    else{
        temText = text.slice(0,value)
        temText = temText.map(function(item){
            return `<p class="result w-[70%] font-light  mt-4 text-xl">${item}</p>`
        })
